...
def find_edges(img, h_min = 0, h_max = 131, s_min = 0, s_max = 255, v_min = 200, v_max = 255):
    height = img.shape[0]
    width = img.shape[1]
...
def draw_line(image, lane_lines, current_theta):
    ...
    if len(lane_lines) == 2:
        ...
        current_theta = display_base_line(image, current_theta, 2, base_x, base_y)
...
#            current_theta = 0
        ...
        base_x = x2 - x1
        base_y = int(height / float(2))
        current_theta = display_base_line(image, current_theta, 1, base_x, base_y)
...
def line_detect(image, current_theta=0, h_min=0, h_max=131, s_min=0, s_max=255, v_min=205, v_max=255):
    ...
    lines = cv2.HoughLinesP(edges, 3, float(3.14)/float(180), 20, minLineLength=20,maxLineGap=200)

    if lines is not None:
        lane_lines = find_lines(image, lines=lines)
        if current_theta == None:
            current_theta = 0
