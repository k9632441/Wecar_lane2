...
class LaneFollower(object):
    ...
    def follow_lane(self, frame):
        ...
        angle,final_frame = self.steer(frame, lane_lines)

        return angle,final_frame
    ...
    def steer(self, frame, lane_lines):
        ...
        if len(lane_lines) == 0:
            ...
            return self.curr_steering_angle,frame
...
def detect_edges(frame):
    ...
    show_image("hsv", hsv)
    lower_blue = np.array([39, 0, 184])
    upper_blue = np.array([221, 255, 255])
...
def detect_line_segments(cropped_edges):
    ...
    line_segments = cv2.HoughLinesP(cropped_edges, rho, angle, min_threshold, np.array([]), minLineLength=50,maxLineGap=200)
...
#    frame = cv2.resize(frame, dsize=(640, 480))
    angle,combo_image = lane_follower.follow_lane(frame)
